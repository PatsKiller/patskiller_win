        ...
        private async void BtnConnect_Click(object? s, EventArgs e)
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    _lblDeviceBanner.Text = $"Selected Device: {device.Name} ({device.Vendor}) — Connected";
                    Log("success", $"Connected to {device.Name}");

                    // If the probe provided VIN/VBATT, surface it immediately (operators like feedback).
                    ...
                    {
                        // Keep VIN field readable, but don't treat it as an error (VIN may be unavailable).
                        _lblVin.Text = "VIN: —————————————————";
                ...
                else
                {
                    ...
                    _lblDeviceBanner.Text = $"Selected Device: {device.Name} ({device.Vendor}) — Connection Failed";
