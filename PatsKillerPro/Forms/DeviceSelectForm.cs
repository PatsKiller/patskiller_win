...
namespace PatsKillerPro.Forms
{
    ...
    public sealed class DeviceSelectForm : Form
    {
        ...
        private void TryCommitSelection()
        {
            ...
            {
                Device = row.Device,
                Vin = string.IsNullOrWhiteSpace(row.Vin) || row.Vin == "—" || row.Vin == "VIN unavailable" ? null : row.Vin,
            ...
            try
            {
                return await Task.Run(async () =>
                {
                    ...
                    try
                    {
                        ...
                        // Read VIN + VBATT (VIN may be unavailable; device is still usable)
                        ...
                        var status = string.IsNullOrWhiteSpace(vin) ? "VIN unavailable" : "Connected";
